...
class ENet_model(object):
    ...
    def __init__(self, model_id, img_height=512, img_width=1024, batch_size=1):
        self.model_id = model_id

        self.project_dir = "/home/ipcvg/tensorflow_enet/segmentation/"
    ...
    def add_logits(self):
        ...
        network, pooling_indices_1 = self.encoder_bottleneck_regular(x=network,
                    ...
                    scope="bottleneck_1_0", downsampling=True,name="layer1")
        print (network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                    drop_prob=self.early_drop_prob_ph, scope="bottleneck_1_1",name="layer1_1")
        print (network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                    drop_prob=self.early_drop_prob_ph, scope="bottleneck_1_2",name="layer1_2")
        print (network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                    drop_prob=self.early_drop_prob_ph, scope="bottleneck_1_3",name="layer1_1")
        print (network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=64,
                    drop_prob=self.early_drop_prob_ph, scope="bottleneck_1_4",name="layer1_1")
        print (network.get_shape().as_list())
