...
results_dir = model.project_dir + "motion9/"
path=results_dir 
...
def segmentation(img):
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        saver.restore(sess, project_dir + "training_logs/model_111_epoch_82.ckpt")
...
start=t.time()
cap = cv2.VideoCapture('input_videos/cut7.mp4')
allframes=[]
lines=[]
angles=[]
ret,frame=cap.read()
...
for i in range(len(allframes)):
	...
	if(abs(mean2-mean1)>1.2):
		...
		track_img1=0.2*frame+0.8*track_img #0.3*pred_img_color
               #track_img1 = cv2.addWeighted(track, alpha,track_img1, beta, 0.0)
